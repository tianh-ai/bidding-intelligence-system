...
# 检查后端服务
test_api "后端服务运行" "lsof -i :18888 | grep LISTEN"

# Auth API测试
test_api "登录API - Admin" "curl -s -X POST http://localhost:18888/api/auth/login -H 'Content-Type: application/json' -d '{\"username\":\"admin\",\"password\":\"admin123\"}' | grep -q '\"role\":\"admin\"'"

test_api "登录API - 普通用户" "curl -s -X POST http://localhost:18888/api/auth/login -H 'Content-Type: application/json' -d '{\"username\":\"test\",\"password\":\"test123\"}' | grep -q '\"role\":\"user\"'"

# Prompts API测试
test_api "提示词列表API" "curl -s http://localhost:18888/api/prompts/templates | grep -q '\['"
...
test_api "前端服务运行" "lsof -i :13000 | grep LISTEN"

test_api "前端首页加载" "curl -s http://localhost:13000 | grep -q '投标智能系统'"

test_api "前端资源加载" "curl -s http://localhost:13000/src/main.tsx | grep -q 'import'"
...
# 获取admin token
ADMIN_TOKEN=$(curl -s -X POST http://localhost:18888/api/auth/login \
...
if [ -n "$ADMIN_TOKEN" ]; then
    ...
    # 测试提示词CRUD
    test_api "创建提示词" "curl -s -X POST http://localhost:18888/api/prompts/templates \
        -H 'Content-Type: application/json' \
        -H 'Authorization: Bearer $ADMIN_TOKEN' \
        -d '{\"title\":\"测试提示词\",\"category\":\"其他\",\"content\":\"测试内容\"}' | grep -q 'id'"
    
    # 获取提示词列表
    PROMPT_LIST=$(curl -s http://localhost:18888/api/prompts/templates)
